								...
								sql.forEach( TRACE, "SELECT * FROM app.files WHERE Name LIKE ? ", filename , file => {		// regulate requested file(s)
									...
									function chipFile( file , ctx ) { 
										...
										chipper(sql, pipe, voxctx => {  // get voxel context for each voxel being chipped
											...
											function loadChip(chip,aoicase,cb) { // Load chip with Npixels then callback(cb).  Auto-forecasting when needed.
												...
												FS.stat(impath, err => { // check if chip in cache
													if (err)  // not in cache
											...
											if (image = voxctx.image) 
												loadChip(  image, job, job => {
													queueChip(job);
												});
